  ...
  return (
    <section className="pt-24 pb-16 px-6 lg:px-20 relative overflow-hidden flex items-center" id="home" style={{ minHeight: '85vh' }}>
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        {/* Description */}
        <motion.p 
          className={`mb-10 max-w-5xl text-lg leading-relaxed transition-colors duration-500 ${
            theme === "dark" ? "text-white/70" : "text-black/70"
          }`}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          {t.highlights.map((highlight, index) => (
            <motion.div
              key={highlight.title}
              className={`glass-strong group relative p-6 rounded-2xl overflow-hidden transition-all duration-500 ${
                theme === "dark"
        ...
        >
          ...
          <div className="flex flex-wrap gap-3">
            {t.skills.map((skill, index) => (
              <motion.span
                key={skill}
                className={`glass px-5 py-2 rounded-full transition-all duration-300 text-sm uppercase tracking-wide ${
                  theme === "dark"
